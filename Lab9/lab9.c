...
int main(void)
{
    ...
    // Create the hash table array and initialize it
    struct HashType *hashTable = (struct HashType *)malloc(sizeof(struct HashType) * HASH_SIZE);
    for (int i = 0; i < HASH_SIZE; i++)
    {
        hashTable[i].head = NULL; // Initialize each element to NULL
    }
    ...
    // Free the memory allocated for the hash table and linked lists
    for (int i = 0; i < HASH_SIZE; i++)
    {
